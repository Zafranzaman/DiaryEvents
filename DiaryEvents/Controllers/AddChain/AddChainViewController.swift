...
class AddChainViewController: UIViewController,UITextFieldDelegate {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        self.title = "Create Chain Event"
        tableview.delegate = self
        tableview.dataSource = self
        Data = db.getAllInfo()
        ChainTitle.delegate = self
    }
    @IBAction func SaveChainData(_ sender: Any) {
        tablecreate()
        print(selectedDidValues)
        
        guard let chainTitlev = self.ChainTitle.text, chainTitlev != "" else {
            let alert = UIAlertController(title: "Alert!!!", message: "Title Is empty", preferredStyle: .alert)
